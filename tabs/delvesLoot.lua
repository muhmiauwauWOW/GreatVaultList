...
function GreatVaultListDelvesLootListMixin:OnLoad()
	...
	GreatVaultListDelvesLootListMixin:SetKeysState(keysState)
...
function GreatVaultListDelvesLootListMixin:GetHelpConfig()
	...
	local helpConfig = {
		...
		[2] = { ButtonPos = { position = "CENTER" },  HighLightBox = { x = width - 210, y = -9, width = 210, height = 30 },	ToolTipDir = "DOWN",  ToolTipText = test2},
	}

	return helpConfig
end

function GreatVaultListDelvesLootListMixin:AddConfig(category)
	

	L["opt_tab_showKeys_name"] = "Show Keys"
	L["opt_tab_showKeys_desc"] = ""

	local setting = Settings.RegisterAddOnSetting(category, "showKeys", "showKeys", GreatVaultList.db.global.Options.tabs[self.id], "boolean", L["opt_tab_showKeys_name"], true)
	setting:SetValueChangedCallback(function(s)
		local value = s:GetValue()
		self:SetKeysState(value)
	end)

	Settings.CreateCheckbox(category, setting, L["opt_tab_showKeys_desc"])
end
